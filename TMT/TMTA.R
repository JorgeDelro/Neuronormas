...
db$tmta_percentil_range <- with (db, ifelse (
  ...
  db$tmta_education_years_adj <- with(db, ifelse(
    is.na(db$tmta_scale_score), NA, ifelse (
  ...
  # NSSae
  db$tmta_NSSae <- with(db, ifelse(
    is.na(db$tmta_education_years_adj), NA, ifelse (
      !is.na(db$tmta_education_years_adj), db$tmta_scale_score - (-0.21832*(db$tmta_education_years_adj-12)) )))
