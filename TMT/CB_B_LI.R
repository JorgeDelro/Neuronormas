...
# 
# 
CB_B_LI <- function(score, age, education_years){
  
  CB_B_LI_db <- data.frame(score = score, age = age, education_years = education_years)
  CB_B_LI_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(CB_B_LI_db)) {
    res <- CB_B_LI_scale_score(score = CB_B_LI_db[i, "score"], 
                              age = CB_B_LI_db[i, "age"],
                              education_years = CB_B_LI_db[i, "education_years"])
...
CB_B_LI_scale_score <- function(score, age, education_years) {
  ...
  if(db$age >= 50  & db$age < 57) {
    ...
    db$CB_B_LI_scale_score <- with ( db, ifelse ( 
...
db$CB_B_LI_percentil_range <- with (db, ifelse (
  is.na(db$score), NA, ifelse (
...
db$CB_B_LI_percentil_range <- with (db, ifelse (
  ...
  db$CB_B_LI_education_years_adj <- with(db, ifelse(
    is.na(db$CB_B_LI_scale_score), NA, ifelse (
    db$education_years >= 0  & db$education_years < 1, db$CB_B_LI_scale_score + 2, ifelse(
     db$education_years >= 1  & db$education_years <= 6, db$CB_B_LI_scale_score + 1, ifelse(
      db$education_years >= 7  & db$education_years <= 12, db$CB_B_LI_scale_score, ifelse(
        db$education_years >= 13  & db$education_years <= 17, db$CB_B_LI_scale_score - 1, ifelse(
          db$education_years >= 18  & db$education_years <= 20, db$CB_B_LI_scale_score - 2, ifelse(
            
            ))))))) )
  
  
  # NSSae
  db$CB_B_LI_NSSae <- with(db, ifelse(
    is.na(db$CB_B_LI_education_years_adj), NA, ifelse (
      !is.na(db$CB_B_LI_education_years_adj), db$CB_B_LI_scale_score - (0.17787*(db$CB_B_LI_education_years_adj-12)) )))
