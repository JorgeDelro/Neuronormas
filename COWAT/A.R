...
# 
a <- function(score, age, education_years){
  
  a_db <- data.frame(score = score, age = age, education_years = education_years)
  a_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(a_db)) {
    res <- a_scale_score(score = a_db[i, "score"], 
                              age = a_db[i, "age"],
                              education_years = a_db[i, "education_years"])
...
a_scale_score <- function(score, age, education_years) {
  ...
  if(db$age >= 50  & db$age < 57) {
    ## 50-56
    # Scale_Score
    db$a_scale_score <- with ( db, ifelse (
...
db$a_percentil_range <- with (db, ifelse (
...
db$a_percentil_range <- with (db, ifelse (
  ...
  db$education_years_adj <- with(db, ifelse(
    db$education_years >= 0  & db$education_years < 1, db$a_scale_score + 3, ifelse(
    db$education_years >= 1  & db$education_years <= 4, db$a_scale_score + 2, ifelse(
    db$education_years >= 5  & db$education_years <= 8, db$a_scale_score + 1, ifelse(
      db$education_years >= 9  & db$education_years <= 12, db$a_scale_score, ifelse(
        db$education_years >= 13  & db$education_years <= 15, db$a_scale_score - 1, ifelse(
          db$education_years >= 16  & db$education_years <= 19, db$a_scale_score - 2, ifelse(
            db$education_years > 19  & db$education_years <= 20, db$a_scale_score- 3, ifelse(
            )))))))))
  
  
  # NSSae
  db$NSSae_a <- db$a_scale_score - (0.25483*(db$education_years_adj-12)) 
