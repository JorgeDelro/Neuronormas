...
P <- p_function(score = db$Q_COG_COWAT_P_PRE,
                            age = db$AGE_PRE,
                            education_years = db$EDUCATIONAL_LEVEL)

# Function GORDA
p_function <- function(score, age, education_years){
  
  p <- data.frame(score = score, age = age, education_years = education_years)
  p_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(p)) {
    res <- pscale_score(score = p[i, "score"], 
                              age = p[i, "age"],
                              education_years = p[i, "education_years"])
...
pscale_score <- function(score, age, education_years) {
  ...
  if(db$age >= 50  & db$age < 57) {
    ## 50-56
    # Scale_Score
    db$pscale_score <- with ( db, ifelse (
...
db$ppercentil_range <- with (db, ifelse (
...
db$ppercentil_range <- with (db, ifelse (
  ...
  db$education_years_adj <- with(db, ifelse(
    db$education_years >= 0  & db$education_years <= 2, db$pscale_score + 2, ifelse(
    db$education_years >= 3  & db$education_years <= 7, db$pscale_score + 1, ifelse(
      db$education_years >= 8  & db$education_years <= 12, db$pscale_score, ifelse(
        db$education_years >= 13  & db$education_years <= 16, db$pscale_score - 1, ifelse(
          db$education_years >= 17  & db$education_years <= 20, db$pscale_score - 2, ifelse(
           
            )))))))
  
  
  # NSSae
  db$NSSae_p <- db$pscale_score - (0.22078*(db$education_years_adj-12)) 
