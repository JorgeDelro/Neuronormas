...
# 
COWAT_r <- function(score, age, education_years){
  
  COWAT_r_db <- data.frame(score = score, age = age, education_years = education_years)
  COWAT_r_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(COWAT_r_db)) {
    res <- COWAT_r_scale_score(score = COWAT_r_db[i, "score"], 
                              age = COWAT_r_db[i, "age"],
                              education_years = COWAT_r_db[i, "education_years"])
    COWAT_r_new <- rbind(COWAT_r_new, res)
  }
  
  return(COWAT_r_new[,c("COWAT_r_scale_score", "COWAT_r_percentil_range", "COWAT_r_NSSae")])
}

COWAT_r_scale_score <- function(score, age, education_years) {
