...
FCSRT_TRIAL1 <- FCSRT_TRIAL1_function(score = db$Q_COG_COWAT_FCSRT_TRIAL1_PRE,
                            age = db$AGE_PRE,
                            education_years = db$EDUCATIONAL_LEVEL)

# Function GORDA
FCSRT_TRIAL1_function <- function(score, age, education_years){
  
  FCSRT_TRIAL1 <- data.frame(score = score, age = age, education_years = education_years)
  FCSRT_TRIAL1_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(FCSRT_TRIAL1)) {
    res <- FCSRT_TRIAL1_scale_score(score = FCSRT_TRIAL1[i, "score"], 
                              age = FCSRT_TRIAL1[i, "age"],
                              education_years = FCSRT_TRIAL1[i, "education_years"])
