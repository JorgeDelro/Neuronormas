...
FCSRT_TR <- FCSRT_TR_function(score = db$Q_COG_COWAT_FCSRT_TR_PRE,
                            age = db$AGE_PRE,
                            education_years = db$EDUCATIONAL_LEVEL)

# Function GORDA
FCSRT_TR_function <- function(score, age, education_years){
  
  FCSRT_TR <- data.frame(score = score, age = age, education_years = education_years)
  FCSRT_TR_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(FCSRT_TR)) {
    res <- FCSRT_TR_scale_score(score = FCSRT_TR[i, "score"], 
                              age = FCSRT_TR[i, "age"],
                              education_years = FCSRT_TR[i, "education_years"])
