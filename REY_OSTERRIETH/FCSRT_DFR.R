...
FCSRT_DFR <- FCSRT_DFR_function(score = db$Q_COG_COWAT_FCSRT_DFR_PRE,
                            age = db$AGE_PRE,
                            education_years = db$EDUCATIONAL_LEVEL)

# Function GORDA
FCSRT_DFR_function <- function(score, age, education_years){
  
  FCSRT_DFR <- data.frame(score = score, age = age, education_years = education_years)
  FCSRT_DFR_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(FCSRT_DFR)) {
    res <- FCSRT_DFR_scale_score(score = FCSRT_DFR[i, "score"], 
                              age = FCSRT_DFR[i, "age"],
                              education_years = FCSRT_DFR[i, "education_years"])
    FCSRT_DFR_new <- rbind(FCSRT_DFR_new, res)
  }
  
  return(FCSRT_DFR_new)
}
...
db$FCSRT_DFR_percentil_range <- with (db, ifelse (
  ...
  # NSSae
  db$NSSae_FCSRT_DFR <- db$FCSRT_DFR_scale_score - (0.16171*(db$education_years_adj-12)) 
