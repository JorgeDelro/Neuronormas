...
ROCF_M_DR <- function(score, age, education_years){
  ...
  return(ROCF_M_DR_new[,c("ROCF_M_DR_scale_score", "ROCF_M_DR_percentil_range", "ROCF_M_DR_NSSae")])
}
...
db$ROCF_M_DR_percentil_range <- with (db, ifelse (
  ...
  db$ROCF_M_DR_education_years_adj <- with(db, ifelse(
    is.na(db$ROCF_M_DR_scale_score), NA, ifelse (
  ...
  # NSSae
  db$ROCF_M_DR_NSSae <- with(db, ifelse(
    is.na(db$ROCF_M_DR_education_years_adj), NA, ifelse (
      !is.na(db$ROCF_M_DR_education_years_adj), db$ROCF_M_DR_scale_score - (0.13346*(db$ROCF_M_DR_education_years_adj-12)) )))
