...
# 
FCSRT_TDRT3TR <- function(score, age, education_years){
  
  FCSRT_TDRT3TR_db <- data.frame(score = score, age = age, education_years = education_years)
  FCSRT_TDRT3TR_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(FCSRT_TDRT3TR_db)) {
    res <- FCSRT_TDRT3TR_scale_score(score = FCSRT_TDRT3TR_db[i, "score"], 
                              age = FCSRT_TDRT3TR_db[i, "age"],
                              education_years = FCSRT_TDRT3TR_db[i, "education_years"])
    FCSRT_TDRT3TR_new <- rbind(FCSRT_TDRT3TR_new, res)
  }
  
  return(FCSRT_TDRT3TR_new[,c("FCSRT_TDRT3TR_scale_score", "FCSRT_TDRT3TR_percentil_range", "FCSRT_TDRT3TR_NSSae")])
}
...
FCSRT_TDRT3TR_scale_score <- function(score, age, education_years) {
  ...
  db$FCSRT_TDRT3TR_education_years_adj <- with(db, ifelse(
    is.na(db$FCSRT_TDRT3TR_scale_score), NA, ifelse (
  ...
  # NSSae
  db$FCSRT_TDRT3TR_NSSae <- with(db, ifelse(
    is.na(db$FCSRT_TDRT3TR_education_years_adj), NA, ifelse (
      !is.na(db$FCSRT_TDRT3TR_education_years_adj), db$FCSRT_TDRT3TR_scale_score - (0.12962*(db$FCSRT_TDRT3TR_education_years_adj-12)) )))
