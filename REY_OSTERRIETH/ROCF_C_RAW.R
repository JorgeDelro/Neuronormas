...
ROCF_C_RAW <- ROCF_C_RAW_function(score = db$Q_COG_COWAT_ROCF_C_RAW_PRE,
                            age = db$AGE_PRE,
                            education_years = db$EDUCATIONAL_LEVEL)

# Function GORDA
ROCF_C_RAW_function <- function(score, age, education_years){
  
  ROCF_C_RAW <- data.frame(score = score, age = age, education_years = education_years)
  ROCF_C_RAW_new <- data.frame()
  
  # NSSa
  for (i in 1:nrow(ROCF_C_RAW)) {
    res <- ROCF_C_RAW_scale_score(score = ROCF_C_RAW[i, "score"], 
                              age = ROCF_C_RAW[i, "age"],
                              education_years = ROCF_C_RAW[i, "education_years"])
